...
def get_context(context):
    ...
    context.scholarship = (
        context.scholarships[0]["scholarship"]
        if context.scholarships
        else "You have no scholarship"
    )
    context.invoices = []
    if context.student:
        context.invoices = frappe.get_all(
            "Sales Invoice",
            filters={"custom_student": context.student},
            fields=[
                "name",
                "customer",
                "posting_date",
    ...
    context.invoices = sorted(
        context.invoices, key=lambda x: (x["status"], x["posting_date"]), reverse=True
    )
