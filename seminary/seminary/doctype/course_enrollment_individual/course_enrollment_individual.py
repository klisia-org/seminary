...
class CourseEnrollmentIndividual(Document):
	...
	def get_inv_data_ce(self):
		...
		submitinvoice = frappe.db.get_single_value('Seminary Settings', 'auto_submit_sales_invoice')
		is_audit = self.audit
		...
		cei.cei_si =0 and
		fc.is_audit = %s and
		pep.pep_event = 'Course Enrollment'""", (self.name, is_audit), as_list=1)
		rows = frappe.db.sql("""select count(pep.payer)  
		from `tabCourse Enrollment Individual` cei,  `tabFee Category` fc, `tabpgm_enroll_payers` pep, `tabPayers Fee Category PE` pfc, `tabCustomer Group` cg, `tabItem Price` ip  
		where cei.name = %s and
		cei.program_ce = pfc.pf_pe and
		pep.parent = pfc.name and
		...
		cei.cei_si =0 and
		fc.is_audit = %s and
		pep.pep_event = 'Course Enrollment'""", (self.name, is_audit)) [0] [0]
