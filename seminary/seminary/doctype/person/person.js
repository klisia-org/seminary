...
frappe.ui.form.on("Person", {
	refresh(frm) {
		// Bound before the early return: a tax ID is typed on a new record too,
		// and it is the conversation/location surfaces below that need a saved
		// doc, not this.
		bind_tax_id(frm);
		if (frm.is_new()) return;
		render_conversation(frm);
		render_org_footprint(frm);
		// Location (ADR 068 §7). The coordinates are resolved from the mailing
		// address and never typed, so the form shows what the last lookup
		// concluded rather than four read-only numbers with no explanation.
		// Shared with Partner Organization: public/js/geo_location.bundle.js.
		window.seminary?.renderLocationSummary(frm);
		window.seminary?.bindFormAddressAutocomplete(frm, { country_field: "mailing_country" });
		["Email", "SMS", "In-App"].forEach((channel) => {
			frm.add_custom_button(__(channel), () => compose(frm, channel), __("Compose"));
		});
	},

	// Either country can decide which document is wanted, so both re-bind
	// (ADR 071). Nationality wins; the mailing country is the fallback.
	nationality(frm) {
		bind_tax_id(frm);
	},
...
// Shared with the Partner Organization and Student Applicant forms:
// public/js/tax_id.bundle.js.
function bind_tax_id(frm) {
	window.seminary?.bindTaxIdField(frm, {
		country_fields: ["nationality", "mailing_country"],
		subject: "person",
	});
}
