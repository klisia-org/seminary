...
class CourseLesson(Document):
	...
	def update_lesson_name_in_document(self, section):
		doctype_map = {"Exercise": "Exercise", "Quiz": "Quiz"}
...
def capture_progress_for_analytics(progress, course):
	if progress in [25, 50, 75, 100]:
		capture("course_progress", "seminary", properties={"course": course, "progress": progress})


def get_quiz_progress(lesson):
	lesson_details = frappe.db.get_value(
		"Course Schedule Lesson", lesson, ["body", "content"], as_dict=1
	)
	quizzes = []
...
def get_lesson_info(chapter):
	return frappe.db.get_value("Course Schedule Chapter", chapter, "coursesc")