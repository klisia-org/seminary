...
| ------------------ | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Level**          | `Nível do Programa`                          | The tier (Certificate, Bachelor, Master, Doctorate, Continuing Ed…) and whether the program is _ongoing_.          |
| **Has an end?**    | _(mirrored)_ `Is Ongoing` | Degree-style (graduation, GPA, transcript) vs. ongoing (no end, no graduation). |
| **Progress model** | `Tipo de Inscrição`                          | Measured by **terms** completed (Time-based) or **credits** earned (Credits-based).             |
| **Admissions**     | `Modo de Matrícula`                          | Apply only during published windows (Timed) or any time (Continuous).                           |
...
- **Mandatory on program enrollment** (`pgm_course_reqonenroll`) — the student is
  **automatically enrolled** in this course; no one has to register them by hand.
  This is **separate** from _Mandatory for this program_ above: that flag is about
  _graduating_, this one is about _getting signed up_. A course can be one, the
...
A few things worth understanding about _when_ and _how_ this happens:
...
- **Paid programs still bill normally.** "Mandatory on enrollment" does **not**
  ...
  _Awaiting Payment_ exactly like a hand-made enrollment until the student pays.
...
>
> To push a newly-flagged course to students who are _already_ enrolled, open the
...
> mandatory-on-enrollment course, and it only ever _adds_ — it never removes a
