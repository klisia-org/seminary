		...
		<div class="border-r p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
				...
				" class="flex-1 border-l p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
					...
					<div v-for="discussion in filteredDiscussions" :key="discussion.name"
						...
						<div class="new-reply mt-4 space-y-2">
							<TextEditor :content="discussion.new_reply" @change="discussion.new_reply = $event"
								:placeholder="__('Write a reply...')" :editable="true" :fixed-menu="true"
								:bubble-menu="false" editor-class="prose-sm min-h-[7rem]" />
			...
			<div v-if="canGradeSubmission" class="mt-8 space-y-4">
				...
				<div>
					...
					<TextEditor :content="comments" @change="(val) => { comments = val; isDirty = true; }"
						:placeholder="__('Enter your comments')" :editable="true" :bubble-menu="true"
						editor-class="prose-sm max-w-none min-h-[7rem]" />
				</div>
			</div>
				...
				<div class="flex flex-col">
					...
					<div class="p-5">
						...
						<div v-if="!hasSavedSubmission" class="text-md mb-4">
							...
							<TextEditor :content="original_post"
								@change="(val) => { original_post = val; isDirty = true; }"
								:placeholder="__('Enter your text here...')" :editable="true" :bubble-menu="true"
								editor-class="prose-sm max-w-none min-h-[7rem]" />
			...
			<div v-else class="text-md mb-4 ml-2">
				...
				<div v-for="submission in savedsubmission.data" :key="submission.name || submission.creation">
					...
					<div class="new-reply mt-4 space-y-2">
						<TextEditor :content="submission.new_reply" @change="submission.new_reply = $event"
							:placeholder="__('Enter a prompt for the discussion.')"
							editor-class="prose-sm min-h-[7rem] border border-gray-300 rounded-md p-2" :editable="true"
							:bubble-menu="true" />
						<div class="flex items-center justify-between">
							<div class="flex items-center space-x-3">
