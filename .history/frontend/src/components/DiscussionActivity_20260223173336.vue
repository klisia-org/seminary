				...
				" class="flex-1 border-l p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
					...
					<div v-for="discussion in filteredDiscussions" :key="discussion.name"
						...
						<div class="new-reply mt-4 space-y-2">
							...
							<RichTextInput :ref="(el) => setEditorRef(discussion.name, el)"
								:content="discussion.new_reply || ''" @change="(val) => { discussion.new_reply = val }"
...
const handleReplySubmit = (item) => {
	const editorRef = editorRefs[item.name]
	console.log('Editor ref:', editorRef)
	console.log('Type:', typeof editorRef)
	console.log('Keys:', editorRef ? Object.keys(editorRef) : 'null')
	console.log('$el:', editorRef?.$el)
	const replyContent = editorRef?.getValue?.()
		|| editorRef?.exposed?.getValue?.()
		|| editorRef?.$?.exposed?.getValue?.()
