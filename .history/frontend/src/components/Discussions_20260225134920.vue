<template>
	...
	<div v-if="props.type === 'single' && !loading">
		...
		<!-- FIXED: v-if so it only mounts when opened -->
		<DiscussionModal v-if="showing" v-model="showTopicModal" :title="title" :doctype="props.doctype"
			:docname="props.docname" :show="show" />
	...
	<!-- FIXED: v-if so it only mounts when opened -->
	<DiscussionModal v-if="showTopicModal" v-model="showTopicModal" :title="__('New {0}').format(title)"
		:doctype="props.doctype" :docname="props.docname" v-model:reloadTopics="topics" />
</template>
...
const openTopicModal = () => {
	...
	showing.value = true; // Ensure the modal is shown
	console.log('Opening topic modal with :', { showTopicModal: showTopicModal.value }); // Debugging
}
</script>
