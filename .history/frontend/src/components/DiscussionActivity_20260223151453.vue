				...
				" class="flex-1 border-l p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
					...
					<div v-for="discussion in filteredDiscussions" :key="discussion.name"
						...
						<div class="new-reply mt-4 space-y-2">
							<TextEditor :content="discussion.new_reply" @change="onNewReplyChange"
...
function onNewReplyChange(val) {
	if (!editorReady) return
	discussion.value.new_reply = val
}

function onCommentsChange(val) {
	if (!editorReady) return
	comments.value = val
	isDirty.value = true
}

function onOriginalPostChange(val) {
	if (!editorReady) return
	original_post.value = val
	isDirty.value = true
}

function onSubmissionReplyChange(val) {
	if (!editorReady) return
	submission.value.new_reply = val
}
