...
<div v-else-if="quizSubmission.data" class="border rounded-md p-20 text-center space-y-4">
  ...
  <div class="text-ink-gray-9">
    {{ __(`You got ${Math.ceil(quizSubmission.data.percentage)}% correct answers with a score of ${quizSubmission.data.score} out of ${quizSubmission.data.score_out_of}`) }}
  </div>
  <div class="mt-6">
    <div v-for="(question, qtidx) in detailedQuestions" :key="question.question_name" class="border rounded-md p-5 mb-4">
      ...
      <!-- Correct Answer -->
      <div class="bg-surface-gray-3 rounded-md p-3 mt-4">
        <strong>{{ __('Correct Answer:') }}</strong> {{ question.correct_answer || __('No Correct Answer') }}
      </div>
...
const submitQuiz = () => {
	...
	createSubmission()
	console.log(__('Quiz submitted with Submit Quiz button and answers shown'))
}
