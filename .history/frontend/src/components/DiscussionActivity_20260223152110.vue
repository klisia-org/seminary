		...
		<div class="border-r p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
				...
				" class="flex-1 border-l p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
					...
					<div v-for="discussion in filteredDiscussions" :key="discussion.name"
						...
						<div class="new-reply mt-4 space-y-2">
							<TextEditor :content="discussion.new_reply || ''"
								@change="(val) => updateReply(discussion, val)" :placeholder="__('Write a reply...')"
								:editable="true" :bubble-menu="true" editor-class="prose-sm min-h-[7rem]" />
			...
			<div v-if="canGradeSubmission" class="mt-8 space-y-4">
				...
				<div>
					...
					<TextEditor :content="comments" @change="onCommentsChange" :editable="true" :bubble-menu="true"
						editor-class="prose-sm min-h-[7rem]" />
				</div>
			</div>
				...
				<div class="flex flex-col">
					...
					<div class="p-5">
						...
						<div v-if="!hasSavedSubmission" class="text-md mb-4">
							...
							<TextEditor :content="original_post" @change="onOriginalPostChange" :editable="true"
								:bubble-menu="true" editor-class="prose-sm min-h-[7rem]" />
			...
			<div v-else class="text-md mb-4 ml-2">
				...
				<div v-for="submission in savedsubmission.data" :key="submission.name || submission.creation">
					...
					<div class="new-reply mt-4 space-y-2">
						<TextEditor :content="submission.new_reply" @change="onSubmissionReplyChange"
							:placeholder="__('Write a reply...')" :editable="true" :bubble-menu="true"
							editor-class="prose-sm min-h-[7rem]" />
						<div class="flex items-center justify-between">
							<div class="flex items-center space-x-3">
...
function updateReply(item, val) {
	if (!editorReady) return
	if (val === item.new_reply) return  // Guard against same value
	item.new_reply = val
}

function updateComments(val) {
	if (!editorReady) return
	if (val === comments.value) return
	comments.value = val
	isDirty.value = true
}

function updateOriginalPost(val) {
	if (!editorReady) return
	if (val === original_post.value) return
	original_post.value = val
	isDirty.value = true
}
}
