	...
	<div class="mt-6">
		...
		<div v-for="(reply, index) in replies.data">
			<div class="py-3" :class="{ 'border-b': index + 1 != replies.data.length }">
				...
				<RichTextInput :content="reply.reply" @change="(val) => (reply.reply = val)"
		...
		<RichTextInput v-if="renderEditor && !readOnlyMode" class="mt-5" :id="`new-reply-${props.topic.name}`" :content="newReply" 
