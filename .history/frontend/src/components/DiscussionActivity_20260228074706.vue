			...
			" class="flex-1 border-l p-5 overflow-y-auto h-[calc(100vh-3.2rem)]">
				...
				<div v-for="discussion in filteredDiscussions" :key="discussion.name"
					...
					<div class="new-reply mt-4 space-y-2">
						...
						<RichTextInput :key="editorKey[discussion.name]" :id="editorKey[discussion.name]"
							:content="discussion.new_reply || ''" :placeholder="__('Write a reply...')"
		...
		<div v-else class="text-md mb-4 ml-2">
			...
			<div v-for="submission in savedsubmission.data" :key="submission.name || submission.creation">
				...
				<div class="new-reply mt-4 space-y-2">
					...
					<RichTextInput :id="'line222'" :content="submission.new_reply || ''"
...
const Student = ref('');
const courseName = computed(() => props.courseName || router.currentRoute.value.params.courseName)
...
const discussionReady = computed(() => {
	return !!(courseName.value && discussionID.value && owner.value)
})
console.log('Discussion Ready:', discussionReady.value, 'Course Name:', courseName.value, 'Discussion ID:', discussionID.value, 'Owner:', owner.value)
