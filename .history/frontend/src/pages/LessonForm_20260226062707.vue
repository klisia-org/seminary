...
const renderEditor = (holder, course = null) => {
	const course = lessonDetails.data?.course_title
	console.log('Rendering editor for course:', course); // Debugging: Check course value
	return new EditorJS({
		holder: holder,
...
const lessonDetails = createResource({
	...
	onSuccess(data) {
		if (data.lesson) {
			// Log the raw value to see what Frappe returns
			// console.log('Raw allow_discuss value:', data.lesson.allow_discuss, typeof data.lesson.allow_discuss)

			Object.keys(data.lesson).forEach((key) => {
				// Skip boolean fields here, handle them explicitly below
			...
			// console.log('lesson.allow_discuss after set:', lesson.allow_discuss)
