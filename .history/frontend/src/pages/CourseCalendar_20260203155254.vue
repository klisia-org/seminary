<template>
    ...
    <div class="mt-5 mb-10 w-full px-5">
        ...
        <p class="text-gray-700 mb-5">{{ __('Subscribe to this course calendar to stay updated with events and schedules.') }}</p>

        <div class="space-y-4">
            <div>
                <h4 class="text-lg font-semibold">{{ __('Google Calendar') }}</h4>
                <p class="text-sm text-gray-600">{{ __('Copy the calendar URL and add it to your Google Calendar by selecting "Other Calendars" > "From URL".') }}</p>
            </div>

            <div>
                <h4 class="text-lg font-semibold">{{ __('Outlook Calendar') }}</h4>
                <p class="text-sm text-gray-600">{{ __('Copy the calendar URL and add it to your Outlook Calendar by selecting "Add Calendar" > "Subscribe from Web".') }}</p>
            </div>

            <div>
                <h4 class="text-lg font-semibold">{{ __('Apple Calendar') }}</h4>
                <p class="text-sm text-gray-600">{{ __('Copy the calendar URL and add it to your Apple Calendar by selecting "File" > "New Calendar Subscription".') }}</p>
            </div>
        </div>
