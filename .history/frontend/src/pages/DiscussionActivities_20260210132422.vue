<template>
	...
	<div class="md:w-3/4 md:mx-auto py-5 mx-5">
		<div class="grid grid-cols-3 gap-5 mb-5">
			...
			<FormControl v-model="courseFilter" type="select" :options="coursesListed" :placeholder="__('Course')" />

		</div>
		<ListView v-if="discussions.data?.length" :columns="discussionColumns" :rows="discussions.data" row-key="name"
			:options="{
				showTooltip: false,
				selectable: false,
				getRowRoute: (row) => ({
		...
		<div v-else class="text-center p-5 text-ink-gray-5 mt-52 w-3/4 md:w-1/2 mx-auto space-y-2">
			...
			<div class="leading-5">
				{{
					__(
						'You have not created any discussion activities yet.' +
						' To create a new discussion activity, click on the "New" button above.'
