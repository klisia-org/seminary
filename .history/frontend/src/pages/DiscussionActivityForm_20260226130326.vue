<template>
	...
	<div class="w-3/4 mx-auto py-5">
		...
		<div class="mb-8">
			...
			<div v-if="!promptReady" class="border rounded-md bg-surface-gray-2 min-h-[7rem] p-2">
				<span class="text-gray-400">{{ __('Loading...') }}</span>
			</div>
			<component v-else :is="TextEditor" :content="discussion.prompt" @change="(val) => discussion.prompt = val"
				:editable="true" :fixedMenu="true" :placeholder="__('Enter a prompt for the discussion.')"
				editorClass="prose-sm max-w-none border-b border-x bg-surface-gray-2 rounded-b-md py-1 px-2 min-h-[7rem]" />
		</div>
	</div>
