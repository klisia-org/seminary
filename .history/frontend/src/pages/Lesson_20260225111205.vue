	...
	<div v-if="lesson.data" class="">
		...
		<div class="grid md:grid-cols-[70%,30%] h-screen">
			<div class="border-r container pt-5 pb-10 px-5">
				...
				<div class="mt-20">
					<Discussions v-if="allowDiscussions && lesson.data?.name" :title="__('Questions about this lesson')"
						:doctype="'Course Lesson'" :docname="lesson.data.name" :key="lesson.data.name" type="multi" />
				</div>
			</div>
...
/* Force the entire grid content area below the dialog */
.grid.md\:grid-cols-\[70\%\,30\%\] {
  position: relative;
  z-index: 1;
}

/* Force the sticky header below the dialog too */
header.sticky {
  z-index: 10; /* keep it above content but below dialog */
}

/* Ensure the dialog is above absolutely everything */
[id^="headlessui-dialog"] {
  z-index: 9999 !important;
  position: fixed !important;
}

/* Also ensure the backdrop is above everything */
.bg-surface-modal {
