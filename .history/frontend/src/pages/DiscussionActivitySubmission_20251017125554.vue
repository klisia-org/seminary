...
let studentName = computed(() => submissionlist.data?.find(sub => sub.name === currentSubmission.value)?.student_name);
  let fetchStudentDataWatcher = watchEffect(() => {
    if (currentSubmission.value && studentName.value) {
