			...
			<div class="border-r">
				...
				<div class="py-5">
					<div class="w-5/6 mx-auto">
						...
						<FormControl v-model="lesson.lesson_title" label="Title" class="mb-4" :required="true" />
						<FormControl v-model="lesson.preview" type="text" label="Text for Preview on the Course Page" />
						<FormControl v-model="lesson.allow_discuss" type="checkbox"
							:label="__('Allow Discussions? (Discussions will be automatically disabled for quizzes)')" />
					</div>
	...
	onSuccess(data) {
		if (data.lesson) {
			...
			lesson.include_in_preview = data?.lesson?.include_in_preview
				? true
				: false
			// ADD THIS LINE - convert 0/1 to boolean for checkbox
			lesson.allow_discuss = data?.lesson?.allow_discuss ? true : false
