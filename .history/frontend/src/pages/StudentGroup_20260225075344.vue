<template>
  <div v-if="course.data && instructors.data">
    ...
    <h1 class="text-2xl font-bold mt-4 mb-4 ml-5">{{ __('Student Group Management for ' + course.data.course) }}</h1>
    <h2 class="text-l mb-12 ml-5">{{ __('Student Groups are used to assign students to specific groups within the course, both for assigning grading to specific instructors and for organizing collaborative work.') }}</h2>
...
const breadcrumbs = computed(() => {
  ...
  items.push({
    ...
    route: { name: 'StudentGroup', params: { courseName: props.courseName } }
  })
  return items
})
