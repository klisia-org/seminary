  ...
  <div class="p-5 flex flex-row gap-4 w-full mx-auto">
    ...
    <div class="w-1/4 p-4">
      ...
      <div v-if="isStudent || isInstructor">
        ...
        <div v-if="courseToDoList.length === 0">
          ...
          <h3 v-if="isStudent && !isInstructor" class="text-xl text-center font-semibold text-gray-500 mt-5"> Congrats!
            No assessments to
            do!</h3>
          <h3 v-if="isInstructor" class="text-xl text-center font-semibold text-gray-500 mt-5"> Congrats! No assessments
            to grade!</h3>
        </div>
      </div>
    </div>
    ...
    <div class="w-3/4">
      ...
      <div class="flex gap-4 mb-5">
        ...
        <div>
          <label for="academicTermFilter" class="block text-sm font-medium text-gray-700">{{ __('Academic Term') }}</label>
...
const current_AcademicTerm = createResource({
  ...
  onSuccess: (data) => {
    if (data?.title) {
      ...
      console.log('isInstructor:', isInstructor.value)
    } else {
