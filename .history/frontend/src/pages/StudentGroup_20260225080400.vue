<template>
  <div v-if="course.data && instructors.data">
    ...
    <h2 class="text-l mb-12 ml-5">
      {{
        __(
          'Student Groups are used to assign students to specific groups within the course, both for assigning grading to specific instructors and for organizing collaborative work.'
        )
      }}
    </h2>
    ...
    <div v-if="hasSavedGroups" class="student-group-page">
      ...
      <div
        ...
        class="mx-5 mb-6 p-4 bg-orange-50 border border-orange-200 rounded-lg"
      >
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <FormControl
                ...
                :placeholder="__('Assign to group...')"
                size="sm"
                @change="(val) => assignStudentToGroup(student, val)"
              />
            </div>
          </div>
        </div>
      ...
      <div class="mx-5 space-y-6">
        <div
          v-for="(group, groupName) in groupedStudents"
          :key="groupName"
          class="border rounded-lg overflow-hidden"
        >
          <!-- Group Header -->
          <div
            class="bg-surface-gray-2 px-4 py-3 flex items-center justify-between"
          >
            <div>
          ...
          <div class="divide-y">
            ...
            >
              ...
              <div class="flex items-center gap-2">
                ...
                <FormControl
                  type="select"
                  :options="getMoveOptions(groupName)"
                  :modelValue="''"
...
function moveStudent(student, fromGroup, toGroup) {
  ...
  if (idx !== -1) {
    // Find the instructor of the target group
    const targetRow = editableGroups.value.find(
      (r) => r.student_group === toGroup
    )
    editableGroups.value[idx].student_group = toGroup
    editableGroups.value[idx].group_instructor =
      targetRow?.group_instructor || ''
  }
}
...
function removeStudentFromGroup(student, fromGroup) {
  ...
  if (idx !== -1) {
    editableGroups.value[idx].student_group = null
    editableGroups.value[idx].group_instructor = null
  }
}
...
async function saveManualChanges() {
  ...
  try {
    ...
    originalGroupsSnapshot.value = JSON.stringify(editableGroups.value)
  } catch (error) {
