...
def check_erpnext():
    ...
    if status["errors"]:
        frappe.throw(
            _("ERPNext setup is incomplete. Please complete the setup before installing {0}").format("Your App"),
...
def check_erpnext_installed():
    ...
    if "erpnext" not in installed_apps:
        frappe.throw(
            _("ERPNext must be installed before installing {0}").format("Your App"),
