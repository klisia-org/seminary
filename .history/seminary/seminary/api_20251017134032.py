...
@frappe.whitelist()
def get_user_discussion_replies(course_name: str | None = None, discussion_id: str | None = None, owner: str | None = None):
	replies = frappe.db.sql("""
		select r.reply, r.reply_attach, r.member_name, r.owner, r.creation, r.reply_dt 
		from `tabDiscussion Submission Replies` r, `tabDiscussion Submission` s 
		where s.coursesc = %s and s.disc_activity = %s and r.parent = s.name and r.owner = %s
		order by r.creation asc""", (course_name, discussion_id, owner), as_dict=1)
	return replies
