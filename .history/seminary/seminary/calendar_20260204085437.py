...
def course_ics(course_schedule: str | None = None, token: str | None = None):
    ...
    meeting_dates = frappe.db.sql(
        """
        select mt.cs_fromtime, mt.cs_totime, mt.cs_meetdate, 'Class' as Activity, cs.room
from `tabCourse Schedule Meeting Dates` mt, `tabCourse Schedule` cs
where mt.parent = 'CHH532 - Introduction to Church History-2025-2026 (SP26)-0491' and
mt.parent = cs.name %s
        """,
        (course_schedule,),
        as_dict=True,
