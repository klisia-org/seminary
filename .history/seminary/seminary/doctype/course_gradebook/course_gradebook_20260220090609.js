...
frappe.ui.form.on('Course Gradebook', {
	...
	render_table: function(frm) {
		...
		let course = frm.doc.course_schedule;
		frappe.call({
	...
	get_marks: function(frm, criteria_list) {
		...
		var result_table = $(frappe.render_template('course_gradebook', {
			frm: frm,
			students: frm.doc.students,
			criteria: criteria_list,
			max_total_score: weightage
		}));
			...
			if (Object.keys(student_scores["assessment_details"]).length === criteria_list.length) {
				...
				frappe.call({
					...
					callback: function(r) {
						...
						$(link_span).find("a").attr("href", "/desk/assessment-result/"+assessment_result.name);
					}
				});
			}
		});
