...
def course_ics(course_schedule=None, token=None):
    ...
    meeting_dates = frappe.db.sql(
        """
        select mt.cs_fromtime, mt.cs_totime, mt.cs_meetdate, cs.room, creation
    ...
    for meeting in meeting_dates:
        ...
        # Add DTSTAMP property to the event with proper UTC handling
        event.created = convert_to_ics_datetime(meeting.creation)
    ...
    for assignment in assignments:
        ...
        event.created = convert_to_ics_datetime(assignment.creation)
        calendar.events.add(event)
