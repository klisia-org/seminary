...
def course_ics(course_schedule: str | None = None, token: str | None = None):
    ...
    meeting_dates = frappe.db.sql(
        """
        select mt.cs_fromtime, mt.cs_totime, mt.cs_meetdate, cs.room, cs.name
...
mt.parent = cs.name
    ...
    assignments = frappe.db.sql(
        """
		select assignment, parent, due_date, parent from `tabScheduled Course Assess Criteria` where parent = %s
        """,
        (course_schedule,),
        as_dict=True,
    )
    ...
    for meeting in meeting_dates:
        ...
        event.name = f"{meeting.name} (_( Class Session))"
        event.description = f"Course Schedule: {meeting.name}\nDate: {meeting.cs_meetdate}\nTime: {meeting.cs_fromtime} - {meeting.cs_totime}"
        if meeting.room:
    ...
    for assignment in assignments:
        ...
        event.name = (
            f"{assignment.parent} - {assignment.assignment} (_( Assignment Due))"
        )
        event.description = f"Course Schedule: {assignment.parent}\nAssignment: {assignment.assignment}\nDue Date: {assignment.due_date}"
        calendar.events.add(event)
