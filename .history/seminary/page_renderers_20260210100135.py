"""Custom page renderers for Seminary ERP app.
...
class CoursePage(BaseRenderer):
    ...
    def render(self):
        if "learn" in self.path:
            ...
            frappe.flags.redirect_location = f"/seminary/courses/{course_name}/learn/{chapter_number}-{lesson_number}"
            return RedirectPage(self.path).render()

        elif len(self.path.split("/")) > 1:
            course_name = self.path.split("/")[1]
            frappe.flags.redirect_location = f"/seminary/courses/{course_name}"
            return RedirectPage(self.path).render()

        else:
            frappe.flags.redirect_location = "/seminary/courses"
            return RedirectPage(self.path).render()
