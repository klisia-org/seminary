...
def setup_fixtures():
    records = [
        ...
        # Customer Group Records
        {
            ...
            "default_price_list": __("Standard Selling"),
        },
        {
            "doctype": "Customer Group",
        ...
        # UOM
        {"doctype": "UOM", "uom_name": __("Academic Event"), "must_be_whole_number": 0},
        {"doctype": "UOM", "uom_name": __("Credit hour"), "must_be_whole_number": 0},
    ]
    make_records(records)


def create_studentappl_role():
    if not frappe.db.exists("Role", __("Student Applicant")):
        frappe.get_doc(
            {"doctype": "Role", "role_name": __("Student Applicant"), "desk_access": 0}
        ).save()


def create_student_role():
    if not frappe.db.exists("Role", __("Student")):
        frappe.get_doc(
            {"doctype": "Role", "role_name": __("Student"), "desk_access": 0}
        ).save()


def create_registrar_role():
    if not frappe.db.exists("Role", __("Registrar")):
        frappe.get_doc(
            {"doctype": "Role", "role_name": __("Registrar"), "desk_access": 1}
        ).save()


def create_instructor_role():
    if not frappe.db.exists("Role", __("Instructor")):
        frappe.get_doc(
            {"doctype": "Role", "role_name": __("Instructor"), "desk_access": 1}
        ).save()
