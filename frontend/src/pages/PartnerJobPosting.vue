<template>
	...
	<div v-else-if="!apps.data?.length" class="mt-20 text-center text-ink-gray-5">{{ __('No applications yet.') }}</div>
	...
	<div v-else class="mx-auto w-full max-w-3xl px-3 py-3 sm:px-5">
		<div class="mb-2 flex items-center gap-3 px-2 text-xs font-medium text-ink-gray-5">
			...
			<div class="ml-auto flex items-center gap-4">
				<button class="flex items-center gap-1 hover:text-ink-gray-7" @click="setSort('average_rating')">
					{{ __('Rating') }}<component :is="sortIcon('average_rating')" class="size-3" />
				</button>
				<button class="flex items-center gap-1 hover:text-ink-gray-7" @click="setSort('submission_date')">
					{{ __('Submitted') }}<component :is="sortIcon('submission_date')" class="size-3" />
				</button>
			</div>
		</div>
		<ul class="divide-y divide-outline-gray-1">
			<li v-for="a in apps.data" :key="a.name">
...
const apps = createResource({
	...
	makeParams: () => ({ opening: props.name, status: status.value, sort_by: sortBy.value, sort_dir: sortDir.value, query: query.value }),
	auto: true,
})
const refetch = debounce(() => apps.reload(), 250)
watch([query, status, sortBy, sortDir], refetch)
